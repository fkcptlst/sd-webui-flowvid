"""
Testing script for StableDiffusion. This script renders a multi-frame video from a set of guide frames.
"""
# Beta V0.72
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        reference_imgs = gr.UploadButton(label="Upload Guide Frames", file_types=['.png', '.jpg', '.jpeg'], live=True,
                                         file_count="multiple")
        ...
        return [append_interrogation, reference_imgs, first_denoise, third_frame_image, color_correction_enabled,
                unfreeze_seed, loopback_source]

    def run(self, p, append_interrogation, reference_imgs, first_denoise, third_frame_image, color_correction_enabled,
            unfreeze_seed, loopback_source):
        freeze_seed = not unfreeze_seed

        loops = len(reference_imgs)  # total number of frames to generate, i.e. len of guide frames

        processing.fix_seed(p)
        batch_count = p.n_iter  # number of iterations per frame?
        ...
        for n in range(batch_count):
            ...
            for i in range(loops):
                ...
                if (i > 0):  # If not the first frame
                    ...
                    if third_frame_image != "None" and i > 1:  # If not the first or second frame
                        ...
                        p.control_net_input_image = msk  # TODO check
                        ...
                        p.image_mask = latent_mask  # TODO check
